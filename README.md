...
- **Redis 内存双层保护（防 OOM）（2026-09-25）**：此前 Redis 容器无内存上限（Docker Memory limit=0），RSS 3.16G 而主机仅 5.8G、可用 1.6G，存在 OOM 风险。现改为三层防护——① Redis 层：`maxmemory 3.5GB` + `volatile-lru`（只淘汰带 TTL 的日志/告警/AI 历史，settings/users/索引集合无 TTL 永不被淘汰）+ `maxmemory-samples 5` + `maxmemory-clients 5%`（限制客户端缓冲）+ `lazyfree-lazy-eviction yes`（异步释放不阻塞写入）；② 容器层：`mem_limit: 4g` + `memswap_limit: 4g`（硬上限并禁用 swap 兜底）；③ 内核层：`vm.overcommit_memory=1`（持久化到 `/etc/sysctl.d/99-redis-overcommit.conf`，避免 AOF 重写 fork 的写时复制导致 fork 失败）。实测配置生效、数据完整（timeline 160 万条不变）、应用无缝重连
